...
func (r *PerPodVerticalRecommender) Recommend(def *pb.RecommenderDefinition, state *pb.ControlMetrics) *pb.RecommenderVote {
	...
	var podRecs []*pb.PodResourceRecommendation
	...
	for podName, podM := range state.PodMetrics {
		...
		podRecs = append(podRecs, &pb.PodResourceRecommendation{
			PodName:       podName,
...
func parseConfig(def *pb.RecommenderDefinition) (*config, error) {
	...
	if resourceType == "" {
		...
		if strings.Contains(lowerMetric, "memory") || strings.Contains(lowerMetric, "mem") {
			resourceType = "memory"
		} else {
			resourceType = "cpu"
		}
